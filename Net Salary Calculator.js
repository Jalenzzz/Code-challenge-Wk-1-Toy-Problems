...
function getTaxRate(salary) {
    ...
    if (salary > 500000.00 && salary <= 799999.99){
        return 32.5;
    }
...
function nhifDeductible(income){
    if (income <= 5999) {
        return 150;
    }
...
function NetSalary(salary){
    ...
    const nhif = nhifDeductible(salary);
...
function main() {
    rl.question("Enter your gross salary\n", (salary) => {
        ...
        if (isNaN(s)) {
            console.log("Please enter a valid numerical salary.\n");
            main();
            return;
        }
        console.log("If your gross salary is ",salary, " Ksh,", "your net salary is ", NetSalary(salary), "Ksh. I've paid my taxes!");
        rl.close();
    });
}
